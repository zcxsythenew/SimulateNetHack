...
Square::Square(const int &x, const int &y, const int &height, const int &width, const int &walls, const int &traps) : uplimit(y), downlimit(y + height - 1), leftlimit(x), rightlimit(x + width - 1), height(height), width(width)
{
	srand(time(NULL));
	map = new char*[height];
